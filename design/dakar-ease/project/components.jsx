...
function PageTransition({ children, screenKey, direction }) {
  ...
  if (!display.animating)
    return <div style={{ ...base, position: 'relative' }}>{display.current}</div>;
      ...
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            ...
            <span
              style={{ fontSize: 10, fontWeight: isActive ? 600 : 400, letterSpacing: '0.01em' }}
            >
              {tab.label}
            </span>
            {isActive && (
    ...
    >
      <div
        style={{
          position: 'relative',
          height: 140,
          background: `linear-gradient(135deg, oklch(0.7 0.12 ${hue}), oklch(0.55 0.14 ${hue + 30}))`,
          display: 'flex',
          alignItems: 'center',
      ...
      >
        ...
        >
          <svg
            ...
            stroke={isFav ? COLORS.danger : '#999'}
            strokeWidth="2"
          >
      ...
      <div style={{ padding: '12px 14px 14px' }}>
        ...
        <div style={{ fontSize: 12, color: COLORS.textLight, marginBottom: 8 }}>
          {listing.district} · {listing.distance}
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontWeight: 700, fontSize: 16, color: COLORS.primary }}>
            {listing.price.toLocaleString('fr-FR')}{' '}
            <span style={{ fontSize: 12, fontWeight: 400, color: COLORS.textLight }}>
              {listing.currency}
              {listing.period}
            </span>
          </span>
...
function PlaceholderImg({ hue, height, icon, style }) {
  return (
    <div
      style={{
        height: height || 200,
        background: `linear-gradient(135deg, oklch(0.72 0.1 ${hue || 210}), oklch(0.55 0.13 ${(hue || 210) + 35}))`,
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        ...style,

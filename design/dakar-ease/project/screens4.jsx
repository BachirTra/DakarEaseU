  ...
  return LISTINGS.map((l) => {
    ...
    if (crit.type && crit.type !== 'any') {
      ...
      if (crit.type === 'coloc') {
        if (coloc.available) {
          score += 30;
          reasons.push('Colocation disponible');
        }
      } else if (l.type === crit.type) {
        score += 30;
        reasons.push('Type ' + l.type);
      }
    }
    ...
    if (crit.school) {
      ...
      if (sc && (sc.logements || []).includes(l.id)) {
        score += 25;
        reasons.push('Proche ' + sc.name);
      } else if (sc && sc.district === l.district) {
        score += 12;
        reasons.push('Même quartier que ' + sc.name);
      }
    } else if (crit.district && crit.district !== 'any') {
      // Quartier (si pas d'école choisie)
      max += 20;
      if (l.district === crit.district) {
        score += 20;
        reasons.push(l.district);
      }
    }
    // Meublé
    if (crit.furnished && crit.furnished !== 'any') {
      ...
      if (!!l.furnished === wantFurnished) {
        score += 10;
        reasons.push(wantFurnished ? 'Meublé' : 'Non meublé');
      }
    }
    ...
    if (crit.coloc && crit.coloc !== 'any' && crit.type !== 'coloc') {
      ...
      if (coloc.available === wantColoc) {
        score += 15;
        reasons.push(wantColoc ? 'Coloc OK' : 'Privatif');
      }
    }
    // Durée
    max += 10;
    if ((l.dureeMin || 3) <= crit.months) {
      score += 10;
    }
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: COLORS.textLight }}>
                Classés par compatibilité avec ta demande
              </div>
            </div>
          </div>
        </div>
            ...
            {results.map(({ listing, pct, reasons }) => (
              ...
              >
                <div style={{ position: 'relative', width: 110, flexShrink: 0 }}>
                  ...
                  <span
                    style={{
                      ...
                      borderRadius: 10,
                      background:
                        pct >= 75 ? COLORS.secondary : pct >= 50 ? COLORS.accent : COLORS.textLight,
                      color: '#fff',
                    }}
                  >
                    {pct}%
                ...
                <div style={{ padding: '12px 14px', flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: COLORS.textLight, marginBottom: 6 }}>
                    {listing.district} · {listing.type} · {listing.area} m²
                  </div>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 800, color: COLORS.primary }}>
                    {listingMinPrice(listing).toLocaleString('fr-FR')}{' '}
                    <span style={{ fontSize: 10, fontWeight: 400, color: COLORS.textLight }}>
                      CFA/mois
                    </span>
                  </div>
                </div>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '14px 20px' }}>
              ...
              {TYPES.map((tp) => (
                <button
                  ...
                  style={{
                    ...
                    gap: 14,
                    padding: 16,
                    background: crit.type === tp.id ? `${COLORS.primary}0c` : '#fff',
                    border: `2px solid ${crit.type === tp.id ? COLORS.primary : COLORS.border}`,
                    borderRadius: 14,
                    cursor: 'pointer',
                    textAlign: 'left',
                  }}
        ...
        {step === 1 && (
          <>
            ...
            <p style={{ fontSize: 13, color: COLORS.textLight, margin: '0 0 18px' }}>
              Proche de ton école, ou dans un quartier précis.
            </p>
            <div
              style={{
                fontSize: 12.5,
            ...
            <select
              ...
              style={{
                ...
                fontSize: 14,
                background: '#fff',
                color: crit.school ? COLORS.text : COLORS.textLight,
                marginBottom: 22,
              }}
            >
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              <Chip active={crit.district === 'any'} onClick={() => set('district', 'any')}>
                Indifférent
              </Chip>
              {DISTRICTS.map((d) => (
                <Chip key={d} active={crit.district === d} onClick={() => set('district', d)}>
                  {d}
                </Chip>
              ))}
            </div>
        ...
        {step === 2 && (
          <>
            ...
            <p style={{ fontSize: 13, color: COLORS.textLight, margin: '0 0 22px' }}>
              On filtre selon tes moyens et ton séjour.
            </p>
            <div style={{ fontSize: 13, color: COLORS.textLight, marginBottom: 8 }}>
              Budget mensuel max :{' '}
              <strong style={{ color: COLORS.text, fontSize: 16 }}>
                {crit.budget.toLocaleString('fr-FR')} CFA
              </strong>
            </div>
            <input
              type="range"
              ...
              style={{ width: '100%', accentColor: COLORS.primary, marginBottom: 28 }}
            />
            <div style={{ fontSize: 13, color: COLORS.textLight, marginBottom: 10 }}>
              Durée du séjour <span style={{ color: COLORS.textLight }}>(min. 3 mois)</span>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              {[3, 6, 9, 12].map((m) => (
                <Chip key={m} active={crit.months === m} onClick={() => set('months', m)}>
                  {m} mois
                </Chip>
              ))}
            </div>
        ...
        {step === 3 && (
          <>
            ...
            <p style={{ fontSize: 13, color: COLORS.textLight, margin: '0 0 22px' }}>
              Dernière étape avant tes résultats.
            </p>
            <div
              style={{
                fontSize: 12.5,
            ...
            <div style={{ display: 'flex', gap: 8, marginBottom: 24 }}>
              <Chip active={crit.furnished === 'any'} onClick={() => set('furnished', 'any')}>
                Peu importe
              </Chip>
              <Chip active={crit.furnished === 'yes'} onClick={() => set('furnished', 'yes')}>
                Meublé
              </Chip>
              <Chip active={crit.furnished === 'no'} onClick={() => set('furnished', 'no')}>
                Non meublé
              </Chip>
            </div>
            <div
            ...
            >
              Ouvert à la colocation ?
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <Chip active={crit.coloc === 'any'} onClick={() => set('coloc', 'any')}>
                Peu importe
              </Chip>
              <Chip active={crit.coloc === 'yes'} onClick={() => set('coloc', 'yes')}>
                Oui
              </Chip>
              <Chip active={crit.coloc === 'no'} onClick={() => set('coloc', 'no')}>
                Logement privatif
              </Chip>
            </div>
            <div style={{ marginTop: 28, padding: 16, background: COLORS.bg, borderRadius: 14 }}>
              <div
                style={{ fontSize: 12, fontWeight: 700, color: COLORS.textLight, marginBottom: 10 }}
              >
              ...
              {[
                ...
                [
                  'Localisation',
                  crit.school || (crit.district === 'any' ? 'Indifférent' : crit.district),
                ],
...
function AdminDashboard({ onBack }) {
  ...
  const [statuses, setStatuses] = useState(() =>
    Object.fromEntries(LISTINGS.map((l) => [l.id, l.verified ? 'online' : 'review'])),
  );
  const toggleStatus = (id) =>
    setStatuses((s) => ({ ...s, [id]: s[id] === 'online' ? 'review' : 'online' }));
  ...
  const reservations = [
    {
      logement: 'Studio meublé Almadies',
      loc: 'Aminata D.',
      mois: 6,
      montant: '1 080 000',
      statut: 'Confirmée',
    ...
    {
      logement: 'Chambre colocation Fann',
      loc: 'Mor S.',
      mois: 3,
      montant: '255 000',
      statut: 'En cours',
    ...
    {
      logement: 'Chambre meublée Ouakam',
      loc: 'Cheikh N.',
      mois: 3,
      montant: '285 000',
      statut: 'En attente',
  ...
  const statutColor = (s) =>
    s === 'Confirmée' ? COLORS.secondary : s === 'En attente' ? COLORS.accent : COLORS.primary;
  ...
  return (
    ...
    >
        ...
        {tabs.map((t) => (
          <button
            ...
            style={{
              ...
              borderRadius: 20,
              border: 'none',
              background: tab === t.id ? COLORS.text : COLORS.bg,
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px 40px' }}>
                ...
                >
                  <div
                    style={{
                      ...
                      height: 38,
                      borderRadius: 10,
                      background: `oklch(0.92 0.05 ${k.hue})`,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                      fontSize: 18,
                  ...
                  <div
                    style={{ fontSize: 11.5, color: COLORS.text, fontWeight: 600, marginTop: 4 }}
                  >
                    {k.label}
                  </div>
                  ...
                  >
                    <div
                      style={{
                        ...
                        background: i === 5 ? COLORS.primary : `${COLORS.primary}44`,
                      }}
                    ></div>
                    <span style={{ fontSize: 9.5, color: COLORS.textLight }}>
            ...
            <div style={{ background: '#fff', borderRadius: 16, padding: 16 }}>
              ...
              {[
                ...
                <div key={d} style={{ marginBottom: 12 }}>
                  ...
                  >
                    <span style={{ color: COLORS.text, fontWeight: 600 }}>{d}</span>
                    <span style={{ color: COLORS.textLight }}>{p}%</span>
                  </div>
                  <div
                    style={{
                      height: 7,
                  ...
                  >
                    <div
                      style={{ width: p + '%', height: '100%', background: COLORS.secondary }}
                    ></div>
                  </div>
                </div>
              ))}
              ...
              return (
                ...
                >
                  <PlaceholderImg
                    ...
                    height={52}
                    icon="🏠"
                    style={{ width: 52, borderRadius: 10, flexShrink: 0 }}
                  />
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: COLORS.textLight }}>
                      {l.district} · {l.price.toLocaleString('fr-FR')} CFA · {l.reviews} avis
                    </div>
                  </div>
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 11,
                      fontWeight: 700,
                      background: st === 'online' ? `${COLORS.secondary}18` : `${COLORS.accent}18`,
                      color: st === 'online' ? COLORS.secondary : COLORS.accent,
                    }}
                  >
        ...
        {tab === 'demandes' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <div style={{ fontSize: 12, color: COLORS.textLight, marginBottom: 2 }}>
              Demandes étudiantes issues de la recherche guidée
            </div>
            {demandes.map((d, i) => (
              <div
                key={i}
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 700 }}>
                      {d.name}{' '}
                      <span style={{ fontSize: 11, color: COLORS.textLight, fontWeight: 400 }}>
                        · {d.school}
                      </span>
                    </div>
                ...
                >
                  ...
                  <span
                    style={{
                      ...
                      padding: '3px 9px',
                      borderRadius: 20,
                      background: `${statutColor(r.statut)}18`,
